...
**Not a development request** (don't redirect): questions about how things work; reading or explaining existing code; running `/status` / `/dashboard` / `/build-report` / `/quality-check`; git operations; conversation during an active `/start` or `/continue` flow.
