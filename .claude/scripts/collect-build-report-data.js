 ...
 *   Cost & effort      ← generated-docs/reports/workflow-insights-data.json — the
 *                        exact transcript-derived tokens/cost/user-inputs/waits the
 *                        workflow-insights skill writes. Summarised here (null when
...
const INSIGHTS_DATA_REL = 'generated-docs/reports/workflow-insights-data.json'; // written by the workflow-insights skill
...
// ── Cost & user involvement (from the workflow-insights data file) ───────────
// The workflow-insights skill parses the Claude Code transcripts and writes exact
...
function collect(root, gapMin = DEFAULT_GAP_MIN) {
  ...
  const dataQuality = {
    sources: [
      ...
      { name: 'Transcript cost & involvement data', found: !!costEffort, note: costEffort ? `workflow-insights data generated ${(costEffort.generatedAt || '').slice(0, 16).replace('T', ' ')} UTC` : 'not generated — run /build-report (it refreshes it) or /workflow-insights' },
